...
import { markdownReferencePlugin } from '../../../utils/markdownReferencePlugin'
import { ReferenceParser } from '../../../utils/references'
...
const md = new MarkdownIt({
  html: true, // Enable HTML tags for reference links
...
md.use(markdownReferencePlugin)
...
const renderedMarkdown = computed(() => {
  // The markdown-it plugin will handle reference parsing and rendering
...
const handleClick = (event: Event) => {
  ...
  // Check if clicked element or its parent is a reference link
  let referenceElement = target.closest('.reference-link') as HTMLElement
  
  if (referenceElement) {
    event.preventDefault()
    event.stopPropagation()
    
    const reference = ReferenceParser.extractReferenceFromElement(referenceElement)
...
/* Reference link styling - light blue with padding */
.markdown-content :deep(.reference-link) {
  @apply cursor-pointer inline-flex items-center justify-center px-2 py-1 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded text-blue-700 text-xs font-medium transition-colors duration-150;
}

.markdown-content :deep(.reference-link:hover) {
